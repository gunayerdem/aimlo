...
function Header({ title, subtitle, lang, onLangClick }: { title: string; subtitle: string; lang: Lang; onLangClick: () => void }) {
 return (
...
export default function Home() {
 ...
 if (!lang) return "";
 const l = t[lang];
 const m: Record<SetupStep, string> = { map: l.stepMap, agent: l.stepAgent, side: l.stepSide, comp: l.stepComp };
 return m[step];
 }
 ...
 <Header title={l.setupTitle} lang={lang} onLangClick={() => setScreen("lang")} subtitle={l.subtitle} />
 ...
 <Header title={l.roundTitle(roundNum)} lang={lang} onLangClick={() => setScreen("lang")} subtitle={`${setup.map} · ${setup.agent} · ${setup.side === "attack" ? l.sideAttack : l.sideDefense}`} />
 ...
 <Header title="Mentu" lang={lang} onLangClick={() => setScreen("lang")} subtitle={l.scoreTitle} />
 <div className="rounded-2xl border border-white/[0.06] bg-neutral-950/80 backdrop-blur-sm p-6 sm:p-8 space-y-5">
 ...
 <Header title={l.reportTitle} lang={lang} onLangClick={() => setScreen("lang")} subtitle={`${setup.map} · ${setup.agent}`} />
