...
function analyzeRoundPatterns(rounds, _setup) {
    ...
    const enemySitePreference = Object.entries(siteCount).sort((a, b)=>b[1] - a[1]).map(([site, frequency])=>({
            site,
            frequency,
    ...
    const enemyAgentHabits = Object.entries(locationCluster).filter(([, count])=>count >= 2).map(([loc, count])=>({
            agent: "Unknown",
    ...
    let enemyTempo = "mixed";
    if (earlyDeaths > lateDeaths * 1.5) enemyTempo = "fast";
    else if (lateDeaths > earlyDeaths * 1.5) enemyTempo = "slow";
...
function generateEnemyPatterns(allRounds, setup) {
    const patterns = analyzeRoundPatterns(allRounds, setup);
    // Build human-readable summary
    const lines = [];
    if (patterns.enemySitePreference.length > 0) {
        const top = patterns.enemySitePreference[0];
        lines.push(`Düşman ${top.site} tarafını tercih ediyor (son 5 ölümde ${top.frequency} kez, güven: ${top.confidence})`);
    }
    if (patterns.enemyAgentHabits.length > 0) {
        patterns.enemyAgentHabits.forEach((h)=>{
            lines.push(`${h.behavior}`);
        });
    }
    lines.push(`Düşman temposu: ${patterns.enemyTempo}`);
    return {
        sitePreference: patterns.enemySitePreference,
        agentHabits: patterns.enemyAgentHabits,
        tempo: patterns.enemyTempo,
        summary: lines.join("\n")
    };
}
function generateNextRoundPlan(patterns, _setup) {
    ...
    if (patterns.enemySitePreference.length > 0) {
        const topSite = patterns.enemySitePreference[0];
...
function computeMatchInsights(rounds, setup) {
    ...
    if (patterns.enemyTempo === "fast" && patterns.survivalRate < 0.5) {
...
- Enemy site preference: ${patterns.enemySitePreference.map((p)=>`${p.site} (${p.frequency} rounds, ${p.confidence})`).join(", ") || "unknown"}
...
async function POST(request) {
    try {
        ...
        let userId;
        try {
            const auth = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$api$2d$auth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["verifyAuthAndRateLimit"])(request);
            if (!auth.ok) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0igxfo6._.js.map