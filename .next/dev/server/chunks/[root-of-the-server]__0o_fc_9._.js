...
"[project]/lib/ai-knowledge.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
 ...
 */ function loadFile(filePath) {
    try {
        const fullPath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(KNOWLEDGE_DIR, filePath);
 ...
 */ function loadDir(dirPath) {
    try {
        const fullPath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(KNOWLEDGE_DIR, dirPath);
        if (!__TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].existsSync(fullPath)) return "";
        const files = __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].readdirSync(fullPath).filter((f)=>f.endsWith(".md"));
        return files.map((f)=>__TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].readFileSync(__TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(fullPath, f), "utf-8")).join("\n\n");
    } catch (e) {
...
function getAdvancedKnowledge(topics) {
    const validTopics = [
        "advanced-mechanics",
        "economy-mastery",
        "clutch-methodology",
    ...
    return topics.filter((t)=>validTopics.includes(t)).map((t)=>loadFile(`general/${t}.md`)).filter(Boolean).join("\n\n");
}
function buildFeedbackSystemPrompt(map, agent, lang) {
    const core = getCoreKnowledge();
    const mapKnowledge = getMapKnowledge(map);
    const agentKnowledge = getAgentKnowledge(agent);
    const langInstruction = lang === "tr" ? "Tüm feedback'i Türkçe yaz. Doğal, akıcı Türkçe kullan." : "Write all feedback in English. Use natural, fluent English.";
...
function buildReportSystemPrompt(map, agent, lang) {
    const core = getCoreKnowledge();
    const mapKnowledge = getMapKnowledge(map);
    const agentKnowledge = getAgentKnowledge(agent);
    const advanced = getAdvancedKnowledge([
        "pro-analysis",
        "radiant-tips"
    ]);
    const langInstruction = lang === "tr" ? "Tüm raporu Türkçe yaz. Profesyonel koçluk dili kullan." : "Write the entire report in English. Use professional coaching language.";
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0o_fc_9._.js.map