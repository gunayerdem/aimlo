...
"[project]/lib/skill-system.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function calculateSkillProfile(allMatches, previousProfile) {
    // Collect all rounds
    const allRounds = allMatches.flatMap((m)=>m.rounds.filter((r)=>!r.skipped));
    ...
    const survival = Math.min(100, Math.max(1, Math.round(survivalRate * 120 - 10)));
    ...
    const concentrationRatio = totalDeaths > 0 ? maxAtOneSpot / totalDeaths : 0;
    ...
    const positioning = Math.min(100, Math.max(1, diversityScore + antiConcentration));
    ...
    const survivedAndWon = allRounds.filter((r)=>r.survived && r.result === "win").length;
    const survivedTotal = allRounds.filter((r)=>r.survived).length;
    const diedAndWon = allRounds.filter((r)=>!r.survived && r.result === "win").length;
    const diedTotal = allRounds.filter((r)=>!r.survived).length;
    const aliveWinRate = survivedTotal > 0 ? survivedAndWon / survivedTotal : 0.5;
    const deathTradeRate = diedTotal > 0 ? diedAndWon / diedTotal : 0.3; // how often death was "useful"
    const decisionMaking = Math.min(100, Math.max(1, Math.round(aliveWinRate * 60 + deathTradeRate * 40)));
    // === ADAPTABILITY (1-100) ===
    ...
    if (firstHalfRepeats === 0 && secondHalfRepeats === 0) {
        ...
        const improvement = (firstHalfRepeats - secondHalfRepeats) / Math.max(firstHalfRepeats, 1);
        adaptability = Math.round(60 + improvement * 40);
    } else if (secondHalfRepeats > firstHalfRepeats) {
        // Got worse
        adaptability = Math.round(40 - (secondHalfRepeats - firstHalfRepeats) * 5);
    } else {
    ...
    let consistency;
    const roundResults = allRounds.map((r)=>r.result === "win" ? 1 : 0);
    ...
    if (windowAverages.length >= 3) {
        ...
        consistency = Math.min(100, Math.max(1, Math.round(100 - variance * 400)));
    } else {
        consistency = 50;
    }
    // === OVERALL ===
    const overall = Math.round(survival * 0.25 + positioning * 0.2 + decisionMaking * 0.25 + adaptability * 0.15 + consistency * 0.15);
    // === RANK ===
    ...
    // === EXPLANATIONS ===
    const explanations = {
        survival: survival >= 70 ? "Hayatta kalma oranın yüksek — iyi pozisyon alıyorsun" : survival >= 45 ? "Hayatta kalma oranın ortalama — gereksiz ölümlerden kaçın" : "Çok sık ölüyorsun — gereksiz peek ve agresyondan kaçın",
        positioning: positioning >= 70 ? "Farklı pozisyonlar kullanıyorsun — okuması zor" : positioning >= 45 ? "Bazı pozisyonlarda takılıyorsun — çeşitlendir" : "Aynı yerlerden sürekli ölüyorsun — çok öngörülebilirsin",
        decisionMaking: decisionMaking >= 70 ? "Hayatta kaldığında roundları kazanıyorsun" : decisionMaking >= 45 ? "Karar verme ortalama — bazı ölümler gereksiz" : "Ölümlerin çoğu takıma katkı sağlamıyor — trade pozisyonu kur",
        adaptability: adaptability >= 70 ? "Hatalarını hızlı düzeltiyorsun" : adaptability >= 45 ? "Bazı hataları tekrarlıyorsun — farkındalık artır" : "Aynı hataları tekrar ediyorsun — her ölümden ders çıkar",
        consistency: consistency >= 70 ? "Stabil performans gösteriyorsun" : consistency >= 45 ? "Bazı maçlar iyi, bazıları kötü — tutarlılık geliştir" : "Performansın çok dalgalı — mental oyunu güçlendir"
    };
    return {
        overall,
        survival,
...
function countRepeatedDeaths(rounds) {
    const locs = {};
    rounds.filter((r)=>!r.survived && r.deathLocation).forEach((r)=>{
...
function calculateRankProgress(score) {
    const thresholds = [
        0,
        25,
        35,
...
function calculateTrends(allMatches, prev) {
    ...
    const recentRounds = recent.flatMap((m)=>m.rounds.filter((r)=>!r.skipped));
    const olderRounds = older.flatMap((m)=>m.rounds.filter((r)=>!r.skipped));
    function calcSurvival(rounds) {
        return rounds.filter((r)=>r.survived).length / Math.max(rounds.length, 1);
    }
    function calcWinRate(rounds) {
        return rounds.filter((r)=>r.result === "win").length / Math.max(rounds.length, 1);
    }
    const THRESHOLD = 0.08; // 8% change needed to be "improving" or "declining"
    function trend(recentVal, olderVal) {
    ...
    return {
        ...
        positioning: "stable",
        decisionMaking: trend(calcWinRate(recentRounds), calcWinRate(olderRounds)),
        overall: trend((calcSurvival(recentRounds) + calcWinRate(recentRounds)) / 2, (calcSurvival(olderRounds) + calcWinRate(olderRounds)) / 2)
    };
}
...
"[project]/lib/playstyle-system.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function analyzePlaystyle(matches, previousArchetype) {
    ...
    const diedAndWon = allRounds.filter((r)=>!r.survived && r.result === "win").length;
    const tradeSuccessRate = deaths > 0 ? Math.round(diedAndWon / deaths * 100) : 50;
    // Win rate
    ...
    if (deathRate > 0.6 + (previousArchetype === "aggressive_entry" ? -MARGIN : 0) && tradeSuccessRate > 45) {
        archetype = "aggressive_entry"; // Dies a lot but deaths are useful
    } else if (deathRate > 0.6 + (previousArchetype === "chaotic_aggressive" ? -MARGIN : 0) && tradeSuccessRate <= 45) {
        archetype = "chaotic_aggressive"; // Dies a lot, deaths are wasted
    } else if (deathRate < 0.4 + (previousArchetype === "passive_anchor" ? MARGIN : 0) && positionDiversity < 40) {
        archetype = "passive_anchor"; // Rarely dies, stays in same spots
    } else if (deathRate < 0.4 + (previousArchetype === "careful_lurker" ? MARGIN : 0) && positionDiversity >= 40) {
    ...
    if (archetype === "aggressive_entry" && winRate < 0.4) {
        mismatch = {
            detected: true,
            message: "Agresif oynuyorsun ama kazanma oranin dusuk. Entry almadan once utility kullan."
        };
    } else if (archetype === "chaotic_aggressive") {
        mismatch = {
            detected: true,
            message: "Cok oluyorsun ve olumlerin ise yaramiyor. Daha kontrollu oyna, trade pozisyonu kur."
        };
    } else if (archetype === "passive_anchor" && winRate < 0.4) {
        mismatch = {
            detected: true,
            message: "Pasif oynuyorsun ama kazanamiyorsun. Bazen agresif olman gerekiyor."
        };
    }
    // Coach message
    const coachMessages = {
        aggressive_entry: "Entry oynuyorsun — olumlerin cogu takima yariyor. Flash/drone ile entry kaliteni artir.",
        chaotic_aggressive: "Agresyonun kontrolsuz. Her peek'in bir sebebi olmali. Utility kullanmadan girme.",
        passive_anchor: "Iyi anchor oynuyorsun. Retake zamanlamani ve bilgi verme kaliteni gelistir.",
        careful_lurker: "Lurk oyunun iyi ama zamanlamaya dikkat et — takimin senden bilgi bekliyor.",
        trade_player: "Trade pozisyonlarin guclu. Entry fragger'inla koordinasyonu artir.",
        balanced_flex: "Dengeli oynuyorsun. Bir yonunu guclendirmek icin odaklan.",
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$skill$2d$system$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/skill-system.ts [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$playstyle$2d$system$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/playstyle-system.ts [app-client] (ecmascript)");
    ...
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("main", {
        ...
        children: [
            ...
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("section", {
                ...
                children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                    className: `${__TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$design$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ds"].card} overflow-hidden relative`,
                    children: [
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */ function AuthScreen({ lang, onAuth, initialMode, onBackToLanding }) {
    ...
    const [showPasswordConfirm, setShowPasswordConfirm] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(false);
    const al = t[lang];
                                            ...
                                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                ...
                                                children: [
                                                    ...
                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                                                        ...
                                                        tabIndex: -1,
                                                        children: showPasswordConfirm ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("svg", {
                                                            width: "18",
                                                            height: "18",
                                                            viewBox: "0 0 24 24",
                                                            fill: "none",
...
_s1(AuthScreen, "HOER4TxDl/B3yTvEfI683KZvRMs=");
_c14 = AuthScreen;
function Home() {
    _s2();
    ...
    const [historyFilterMap, setHistoryFilterMap] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])("");
    const [historyFilterAgent, setHistoryFilterAgent] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])("");
    const [historyFilterResult, setHistoryFilterResult] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])("all");
    ...
    const [downloadBannerDismissed, setDownloadBannerDismissed] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(false);
    ...
    const agentPerf = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[agentPerf]": ()=>{
            const map = {};
            savedReports.forEach({
                "Home.useMemo[agentPerf]": (r)=>{
                    if (!r.agent) return;
            ...
            return Object.entries(map).map({
                "Home.useMemo[agentPerf]": ([name, d])=>({
                        name,
                        wins: d.wins,
                        total: d.total,
                        wr: d.total > 0 ? Math.round(d.wins / d.total * 100) : 0
                    })
            }["Home.useMemo[agentPerf]"]).sort({
                "Home.useMemo[agentPerf]": (a, b)=>b.total - a.total
            }["Home.useMemo[agentPerf]"]);
        }
    }["Home.useMemo[agentPerf]"], [
        savedReports
    ...
    const mapPerf = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[mapPerf]": ()=>{
            const m = {};
            savedReports.forEach({
                "Home.useMemo[mapPerf]": (r)=>{
                    if (!r.map) return;
                    if (!m[r.map]) m[r.map] = {
                        wins: 0,
                        total: 0
            ...
            return Object.entries(m).map({
                "Home.useMemo[mapPerf]": ([name, d])=>({
                        name,
                        wins: d.wins,
                        total: d.total,
                        wr: d.total > 0 ? Math.round(d.wins / d.total * 100) : 0
                    })
            }["Home.useMemo[mapPerf]"]).sort({
                "Home.useMemo[mapPerf]": (a, b)=>b.total - a.total
            }["Home.useMemo[mapPerf]"]);
        }
    }["Home.useMemo[mapPerf]"], [
        savedReports
    ...
    const aiSummary = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[aiSummary]": ()=>{
            if (savedReports.length === 0) return null;
            // Most common mistake — aggregate from mistake field
            const mistakes = {};
            savedReports.forEach({
                "Home.useMemo[aiSummary]": (r)=>{
                    if (r.mistake) {
            ...
            const topMistake = Object.entries(mistakes).sort({
                "Home.useMemo[aiSummary]": (a, b)=>b[1] - a[1]
            }["Home.useMemo[aiSummary]"])[0];
            // Strength — best agent by win rate (min 2 matches)
            const bestAgent = agentPerf.filter({
                "Home.useMemo[aiSummary]": (a)=>a.total >= 2
            }["Home.useMemo[aiSummary]"]).sort({
                "Home.useMemo[aiSummary]": (a, b)=>b.wr - a.wr
            }["Home.useMemo[aiSummary]"])[0];
            // Improvement area — worst map by win rate (min 2 matches)
            const worstMap = mapPerf.filter({
                "Home.useMemo[aiSummary]": (m)=>m.total >= 2
            }["Home.useMemo[aiSummary]"]).sort({
                "Home.useMemo[aiSummary]": (a, b)=>a.wr - b.wr
            }["Home.useMemo[aiSummary]"])[0];
            return {
    ...
    const aiInsight = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[aiInsight]": ()=>{
            const ll = t[lang ?? "en"];
            if (savedReports.length < 2) return ll.aiInsightNoData;
            const deathSpots = {};
            savedReports.forEach({
                "Home.useMemo[aiInsight]": (r)=>r.rounds?.filter({
                        "Home.useMemo[aiInsight]": (rd)=>!rd.skipped && !rd.survived && rd.deathLocation
                    }["Home.useMemo[aiInsight]"]).forEach({
                        "Home.useMemo[aiInsight]": (rd)=>{
                            deathSpots[rd.deathLocation] = (deathSpots[rd.deathLocation] || 0) + 1;
                        }
                    }["Home.useMemo[aiInsight]"])
            }["Home.useMemo[aiInsight]"]);
            const topDeath = Object.entries(deathSpots).sort({
                "Home.useMemo[aiInsight]": (a, b)=>b[1] - a[1]
            }["Home.useMemo[aiInsight]"])[0];
            const recent = savedReports.slice(0, 5);
            const recentWR = recent.length > 0 ? recent.filter({
                "Home.useMemo[aiInsight]": (r)=>r.won
            }["Home.useMemo[aiInsight]"]).length / recent.length * 100 : 0;
            const mapWR = {};
            savedReports.forEach({
                "Home.useMemo[aiInsight]": (r)=>{
            ...
            const worst = Object.entries(mapWR).filter({
                "Home.useMemo[aiInsight]": (e)=>e[1].t >= 2
            }["Home.useMemo[aiInsight]"]).sort({
                "Home.useMemo[aiInsight]": (a, b)=>a[1].w / a[1].t - b[1].w / b[1].t
            }["Home.useMemo[aiInsight]"])[0];
            let text = "";
            if (lang === "tr") {
    ...
    const problemAreas = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[problemAreas]": ()=>{
            const deathSpots = {};
            savedReports.forEach({
                "Home.useMemo[problemAreas]": (r)=>r.rounds?.filter({
                        "Home.useMemo[problemAreas]": (rd)=>!rd.skipped && !rd.survived && rd.deathLocation
                    }["Home.useMemo[problemAreas]"]).forEach({
                        "Home.useMemo[problemAreas]": (rd)=>{
                            deathSpots[rd.deathLocation] = (deathSpots[rd.deathLocation] || 0) + 1;
                        }
                    }["Home.useMemo[problemAreas]"])
            }["Home.useMemo[problemAreas]"]);
            const topDeath = Object.entries(deathSpots).sort({
                "Home.useMemo[problemAreas]": (a, b)=>b[1] - a[1]
            }["Home.useMemo[problemAreas]"])[0];
            const mapWR = {};
            savedReports.forEach({
                "Home.useMemo[problemAreas]": (r)=>{
                    if (!r.map) return;
                    if (!mapWR[r.map]) mapWR[r.map] = {
                        w: 0,
                        t: 0
            ...
            const worstMap = Object.entries(mapWR).filter({
                "Home.useMemo[problemAreas]": (e)=>e[1].t >= 2
            }["Home.useMemo[problemAreas]"]).sort({
                "Home.useMemo[problemAreas]": (a, b)=>a[1].w / a[1].t - b[1].w / b[1].t
            }["Home.useMemo[problemAreas]"])[0];
            // Pattern: find if any death location appears 3+ times in recent matches
            const recentDeaths = {};
            savedReports.slice(0, 5).forEach({
                "Home.useMemo[problemAreas]": (r)=>r.rounds?.filter({
                        "Home.useMemo[problemAreas]": (rd)=>!rd.skipped && !rd.survived && rd.deathLocation
                    }["Home.useMemo[problemAreas]"]).forEach({
                        "Home.useMemo[problemAreas]": (rd)=>{
            ...
            const repeatingSpot = Object.entries(recentDeaths).filter({
                "Home.useMemo[problemAreas]": (e)=>e[1] >= 3
            }["Home.useMemo[problemAreas]"]).sort({
                "Home.useMemo[problemAreas]": (a, b)=>b[1] - a[1]
            }["Home.useMemo[problemAreas]"])[0];
            return {
    ...
    const webSkillProfile = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[webSkillProfile]": ()=>{
            if (savedReports.length < 2) return null;
            const matchData = savedReports.map({
                "Home.useMemo[webSkillProfile].matchData": (r)=>({
                        won: r.won,
                        map: r.map,
                        agent: r.agent,
                        rounds: (r.rounds || []).map({
                            "Home.useMemo[webSkillProfile].matchData": (rd)=>({
                        ...
                        }["Home.useMemo[webSkillProfile].matchData"])
                    })
            }["Home.useMemo[webSkillProfile].matchData"]);
            return (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$skill$2d$system$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["calculateSkillProfile"])(matchData);
        }
    }["Home.useMemo[webSkillProfile]"], [
        savedReports
    ]);
    // ── Playstyle (from lib) ──
    const webPlaystyle = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[webPlaystyle]": ()=>{
            if (savedReports.length < 3) return null;
            const matchData = savedReports.map({
                "Home.useMemo[webPlaystyle].matchData": (r)=>({
                        won: r.won,
                        agent: r.agent,
                        rounds: (r.rounds || []).map({
                            "Home.useMemo[webPlaystyle].matchData": (rd)=>({
                                    deathLocation: rd.deathLocation,
                                    survived: rd.survived,
                                    skipped: rd.skipped,
                                    result: rd.result
                                })
                        }["Home.useMemo[webPlaystyle].matchData"])
                    })
            }["Home.useMemo[webPlaystyle].matchData"]);
            return (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$playstyle$2d$system$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["analyzePlaystyle"])(matchData);
        }
    }["Home.useMemo[webPlaystyle]"], [
        savedReports
    ]);
    // Match mini insight helper
    const getMatchInsight = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "Home.useCallback[getMatchInsight]": (entry)=>{
            if (!entry.rounds || entry.rounds.length === 0) return null;
            const ll = t[lang ?? "en"];
            const deaths = {};
            entry.rounds.filter({
                "Home.useCallback[getMatchInsight]": (rd)=>!rd.skipped && !rd.survived && rd.deathLocation
            }["Home.useCallback[getMatchInsight]"]).forEach({
                "Home.useCallback[getMatchInsight]": (rd)=>{
                    deaths[rd.deathLocation] = (deaths[rd.deathLocation] || 0) + 1;
                }
            }["Home.useCallback[getMatchInsight]"]);
            const topRepeat = Object.entries(deaths).filter({
                "Home.useCallback[getMatchInsight]": (e)=>e[1] >= 3
            }["Home.useCallback[getMatchInsight]"]).sort({
                "Home.useCallback[getMatchInsight]": (a, b)=>b[1] - a[1]
            }["Home.useCallback[getMatchInsight]"])[0];
            if (topRepeat) {
    ...
    function generateMiniMatchInsight(report) {
        if (!report.rounds || report.rounds.length === 0) return null;
        const deaths = {};
        report.rounds.filter((r)=>!r.skipped && !r.survived && r.deathLocation).forEach((r)=>{
            deaths[r.deathLocation] = (deaths[r.deathLocation] || 0) + 1;
        });
    ...
    function getMatchTagWeb(report) {
        ...
        if (report.won && survRate > 0.55) return {
            label: "Dominant",
            color: "#10b981"
        };
        if (!report.won && survRate < 0.3) return {
            label: lang === "tr" ? "Riskli" : "Risky",
            color: "#ef4444"
        };
        if (report.won) return {
            label: lang === "tr" ? "Kontrollü" : "Controlled",
            color: "#22d3ee"
        };
        return {
            label: lang === "tr" ? "Gelişim" : "Growth",
            color: "#f59e0b"
        };
    }
    ...
    const filteredReports = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[filteredReports]": ()=>{
            let filtered = savedReports;
            if (historyFilterMap) filtered = filtered.filter({
                "Home.useMemo[filteredReports]": (r)=>r.map === historyFilterMap
            }["Home.useMemo[filteredReports]"]);
            if (historyFilterAgent) filtered = filtered.filter({
                "Home.useMemo[filteredReports]": (r)=>r.agent === historyFilterAgent
            }["Home.useMemo[filteredReports]"]);
            if (historyFilterResult === "wins") filtered = filtered.filter({
                "Home.useMemo[filteredReports]": (r)=>r.won
            }["Home.useMemo[filteredReports]"]);
            if (historyFilterResult === "losses") filtered = filtered.filter({
                "Home.useMemo[filteredReports]": (r)=>!r.won
            }["Home.useMemo[filteredReports]"]);
            return filtered;
        }
    ...
    const filteredWinRate = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[filteredWinRate]": ()=>{
            if (filteredReports.length === 0) return 0;
            return Math.round(filteredReports.filter({
                "Home.useMemo[filteredWinRate]": (r)=>r.won
            }["Home.useMemo[filteredWinRate]"]).length / filteredReports.length * 100);
        }
    }["Home.useMemo[filteredWinRate]"], [
        filteredReports
    ]);
    // Unique maps and agents for filter dropdowns
    const uniqueMaps = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[uniqueMaps]": ()=>[
                ...new Set(savedReports.map({
                    "Home.useMemo[uniqueMaps]": (r)=>r.map
                }["Home.useMemo[uniqueMaps]"]).filter(Boolean))
            ].sort()
    }["Home.useMemo[uniqueMaps]"], [
        savedReports
    ]);
    const uniqueAgents = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "Home.useMemo[uniqueAgents]": ()=>[
                ...new Set(savedReports.map({
                    "Home.useMemo[uniqueAgents]": (r)=>r.agent
                }["Home.useMemo[uniqueAgents]"]).filter(Boolean))
            ].sort()
    }["Home.useMemo[uniqueAgents]"], [
        savedReports
    ]);
            ...
            setSetup({
                "Home.useEffect": (prev)=>{
                    ...
                    if (prev.agent) {
                        ...
                        if (newComp.length === prev.teamComp.length && newComp.every({
                            "Home.useEffect": (a, i)=>a === prev.teamComp[i]
                        }["Home.useEffect"])) {
                            return prev;
                        }
                        return {
                        ...
                        const newComp = comp.filter({
                            "Home.useEffect.newComp": (a)=>a
                        }["Home.useEffect.newComp"]);
                        if (newComp.length === prev.teamComp.length && newComp.every({
                            "Home.useEffect": (a, i)=>a === prev.teamComp[i]
                        }["Home.useEffect"])) {
                            return prev;
    ...
    /* DASHBOARD */ if (screen === "dashboard") {
        ...
        return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("main", {
            ...
            children: [
                ...
                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                    className: "relative z-10 mx-auto max-w-3xl px-4 pt-20 pb-12",
                    children: [
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "relative overflow-hidden rounded-xl border border-[#1e2a3a]/60 bg-gradient-to-br from-[#0a1628] via-[#0d1117] to-[#0a0f16] p-8 mb-8",
                            style: {
                                minHeight: 200
                            },
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "pointer-events-none absolute top-0 right-0 w-96 h-96 rounded-full bg-cyan-500/[0.04] blur-[120px]"
                                }, void 0, false, {
                                    fileName: "[project]/app/page.tsx",
                                    lineNumber: 3533,
                                    columnNumber: 13
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "pointer-events-none absolute -bottom-20 -left-20 w-64 h-64 rounded-full bg-blue-500/[0.03] blur-[100px]"
                                }, void 0, false, {
                                    fileName: "[project]/app/page.tsx",
                                    lineNumber: 3534,
                                    columnNumber: 13
                                ...
                                topAgent && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "absolute right-0 top-0 bottom-0 w-1/3 overflow-hidden",
                                    children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("img", {
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: [
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                            className: "text-[10px] font-bold uppercase tracking-[0.15em] text-cyan-400/60 mb-2",
                                            children: "AI-POWERED VALORANT COACH"
                                        }, void 0, false, {
                                            fileName: "[project]/app/page.tsx",
                                            lineNumber: 3549,
                                            columnNumber: 15
                                            ...
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    children: [
                                                        ...
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                            className: "text-xs text-neutral-500 ml-1.5",
                                                            children: l.dashMatches
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3559,
                                                            columnNumber: 19
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    children: [
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                            className: `text-2xl font-black ${winRate >= 50 ? 'text-emerald-400' : 'text-red-400'}`,
                                                            children: savedReports.length > 0 ? `${winRate}%` : '\u2014'
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3563,
                                                            columnNumber: 19
                                                ...
                                                topAgent && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    ...
                                                    children: [
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("img", {
                                                            src: (0, __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$game$2d$data$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["agentImgUrl"])(topAgent.name),
                                                            alt: "",
                                                            className: "w-7 h-7 rounded-lg ring-1 ring-cyan-500/30"
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3569,
                                                            columnNumber: 21
                                                        ...
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                            children: [
                                                                ...
                                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                                    ...
                                                                    children: lang === "tr" ? "En çok oynanan" : "Most played"
                                                                }, void 0, false, {
                                                                    fileName: "[project]/app/page.tsx",
                                                                    lineNumber: 3572,
                                                                    columnNumber: 23
                        ...
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "rounded-xl border border-cyan-500/10 bg-gradient-to-br from-[#0a1628]/90 to-[#0d1117]/95 p-6 mb-6 relative overflow-hidden",
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "pointer-events-none absolute inset-0 bg-gradient-to-r from-cyan-500/[0.03] to-transparent"
                                }, void 0, false, {
                                    fileName: "[project]/app/page.tsx",
                                    lineNumber: 3582,
                                    columnNumber: 13
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: [
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            ...
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    ...
                                                    children: [
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                            className: "w-1 h-5 rounded-full bg-cyan-400",
                                                            style: {
                                                        ...
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                            className: "rounded-md bg-cyan-500/[0.08] border border-cyan-500/15 px-2 py-0.5 text-[9px] font-semibold text-cyan-400/70",
                                                            children: "AI"
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3588,
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    ...
                                                    children: [
                                                        ...
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                            className: "rounded bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 text-[9px] font-bold text-amber-400",
                                                            children: lang === "tr" ? "ORTA G\u00dcVEN" : "MED CONFIDENCE"
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3592,
                                                            columnNumber: 19
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                            className: "text-[15px] leading-relaxed text-neutral-200 mb-4",
                                            children: aiInsight
                                        }, void 0, false, {
                                            fileName: "[project]/app/page.tsx",
                                            lineNumber: 3595,
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                                            className: "rounded-lg bg-cyan-500/[0.08] border border-cyan-500/20 px-4 py-2 text-[11px] font-semibold text-cyan-400 transition hover:bg-cyan-500/15",
                                            children: lang === "tr" ? "Detayl\u0131 Analiz \u2192" : "Detailed Analysis \u2192"
                                        }, void 0, false, {
                                            fileName: "[project]/app/page.tsx",
                                            lineNumber: 3596,
                                            columnNumber: 15
                        ...
                        savedReports.length >= 2 && (problemAreas.deathSpot || problemAreas.worstMap || problemAreas.pattern) && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "mb-8",
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: [
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "rounded-lg border border-red-500/10 bg-gradient-to-b from-red-500/[0.04] to-transparent p-5 relative overflow-hidden",
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-red-500 via-red-400 to-transparent"
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3612,
                                                    columnNumber: 19
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    className: "text-[9px] font-bold uppercase tracking-[0.1em] text-red-400/70 mb-2",
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    ...
                                                    children: problemAreas.deathSpot ? `${problemAreas.deathSpot.count}x ${l.problemDeathDesc}` : l.problemNoData
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3615,
                                                    columnNumber: 19
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "rounded-lg border border-amber-500/10 bg-gradient-to-b from-amber-500/[0.04] to-transparent p-5 relative overflow-hidden",
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-amber-500 via-amber-400 to-transparent"
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3618,
                                                    columnNumber: 19
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    className: "text-[9px] font-bold uppercase tracking-[0.1em] text-amber-400/70 mb-2",
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    className: "text-[11px] text-neutral-500 mt-1",
                                                    children: problemAreas.worstMap ? `${problemAreas.worstMap.wr}% ${l.problemMapDesc}` : l.problemNoData
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3621,
                                                    columnNumber: 19
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "rounded-lg border border-purple-500/10 bg-gradient-to-b from-purple-500/[0.04] to-transparent p-5 relative overflow-hidden",
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-purple-500 via-purple-400 to-transparent"
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3624,
                                                    columnNumber: 19
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    className: "text-[9px] font-bold uppercase tracking-[0.1em] text-purple-400/70 mb-2",
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    className: "text-[11px] text-neutral-500 mt-1",
                                                    children: problemAreas.pattern ? `${problemAreas.pattern.count}x ${l.problemPatternDesc}` : l.problemNoData
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3627,
                                                    columnNumber: 19
                        ...
                        webSkillProfile && webSkillProfile.overall > 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "rounded-xl border border-[#1e2a3a]/60 bg-gradient-to-br from-[#0a1628]/90 to-[#0d1117]/95 p-6 mb-6 relative overflow-hidden",
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full bg-cyan-500/[0.04] blur-[80px]"
                                }, void 0, false, {
                                    fileName: "[project]/app/page.tsx",
                                    lineNumber: 3636,
                                    columnNumber: 15
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: [
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "text-[10px] font-bold uppercase tracking-[0.12em] text-cyan-400 mb-4",
                                            children: lang === "tr" ? "SEV\u0130YE & \u0130LERLEME" : "LEVEL & PROGRESS"
                                        }, void 0, false, {
                                            fileName: "[project]/app/page.tsx",
                                            lineNumber: 3638,
                                            columnNumber: 17
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "text-5xl font-black text-cyan-400 mb-1",
                                            style: {
                                                textShadow: '0 0 30px rgba(34,211,238,0.3)'
                                            },
                                            children: webSkillProfile.overall
                                        }, void 0, false, {
                                            fileName: "[project]/app/page.tsx",
                                            lineNumber: 3639,
                                            columnNumber: 17
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            ...
                                            children: webSkillProfile.explanations?.decisionMaking || ''
                                        }, void 0, false, {
                                            fileName: "[project]/app/page.tsx",
                                            lineNumber: 3643,
                                            columnNumber: 17
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: [
                                        {
                                            label: lang === "tr" ? 'Hayatta Kalma' : 'Survival',
                                            value: webSkillProfile.survival,
                                            color: '#10b981'
                                        },
                                        {
                                            label: lang === "tr" ? 'Pozisyonlama' : 'Positioning',
                                            value: webSkillProfile.positioning,
                                            color: '#3b82f6'
                                        },
                                        {
                                            label: lang === "tr" ? 'Karar Verme' : 'Decision Making',
                                            value: webSkillProfile.decisionMaking,
                                            color: '#22d3ee'
                                        }
                                            ...
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    ...
                                                    style: {
                                                        color: s.color,
                                                        textShadow: `0 0 12px ${s.color}30`
                                                    },
                                                    children: s.value
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "text-[9px] font-semibold uppercase tracking-[0.08em] text-neutral-500 mb-2",
                                                    children: s.label
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3654,
                        ...
                        webPlaystyle && webPlaystyle.archetype !== "unknown" && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "mb-6",
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "flex items-center gap-2 mb-4",
                                    children: [
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                            className: "text-[10px] font-bold uppercase tracking-[0.12em] text-purple-400",
                                            children: lang === "tr" ? "OYUN TARZI" : "PLAYSTYLE"
                                        }, void 0, false, {
                                            fileName: "[project]/app/page.tsx",
                                            lineNumber: 3669,
                                            columnNumber: 17
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "rounded-xl border border-purple-500/10 bg-gradient-to-br from-purple-500/[0.04] to-[#0d1117]/95 p-6 relative overflow-hidden",
                                    children: [
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "pointer-events-none absolute inset-0 bg-gradient-to-r from-purple-500/[0.03] to-transparent"
                                        }, void 0, false, {
                                            fileName: "[project]/app/page.tsx",
                                            lineNumber: 3673,
                                            columnNumber: 17
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            ...
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    ...
                                                    children: webPlaystyle.archetypeLabel
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3675,
                                                    columnNumber: 19
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    className: "text-xs text-neutral-400 leading-relaxed mb-3",
                                                    children: webPlaystyle.archetypeDescription
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3676,
                                                    columnNumber: 19
                                                }, this),
                                                webPlaystyle.mismatch && webPlaystyle.mismatch.detected && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "bg-red-500/[0.08] border border-red-500/20 rounded-lg p-3 mb-3",
                                                    children: [
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                            className: "text-[10px] font-bold uppercase tracking-wider text-red-400 mb-1",
                                                            children: lang === "tr" ? "UYARI" : "WARNING"
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3679,
                                                            columnNumber: 23
                                                        ...
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                            ...
                                                            children: webPlaystyle.mismatch.message
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3680,
                                                            columnNumber: 23
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    ...
                                                    children: webPlaystyle.coachMessage
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3683,
                                                    columnNumber: 19
                            ...
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: [
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                            className: "text-[10px] font-bold uppercase tracking-[0.12em] text-blue-400",
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "rounded-xl border border-[#1e2a3a]/60 bg-gradient-to-br from-[#0a1628]/90 to-[#0d1117]/95 p-6",
                                    children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        ...
                        agentPerf.length > 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "mb-6",
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "flex items-center gap-2 mb-4",
                                    children: [
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "w-1 h-4 rounded-full bg-cyan-400",
                                            style: {
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                            className: "text-[10px] font-bold uppercase tracking-[0.12em] text-cyan-400",
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: agentPerf.map((ap)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "rounded-xl border border-[#1e2a3a]/60 bg-gradient-to-b from-[#0a1628]/80 to-[#0d1117]/90 p-4 text-center relative overflow-hidden transition hover:border-[#2d4a6f]/40 hover:shadow-lg hover:shadow-black/20",
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-cyan-500/40 via-cyan-400/20 to-transparent"
                                                }, void 0, false, {
                                                    fileName: "[project]/app/page.tsx",
                                                    lineNumber: 3727,
                                                    columnNumber: 21
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "h-10 w-10 rounded-xl overflow-hidden bg-black/30 ring-1 ring-white/[0.08] mx-auto mb-2",
                                                    style: {
                                                        filter: 'saturate(1.2)'
                                                    },
                                                    children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("img", {
                                                        src: (0, __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$game$2d$data$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["agentImgUrl"])(ap.name),
                                                        alt: ap.name,
                                                        className: "h-full w-full object-cover",
                                                        loading: "lazy"
                                                    }, void 0, false, {
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                    className: `text-xl font-extrabold tabular-nums ${ap.wr >= 50 ? "text-emerald-400" : "text-red-400"}`,
                                                    style: {
                                                        textShadow: ap.wr >= 50 ? '0 0 10px rgba(16,185,129,0.2)' : '0 0 10px rgba(239,68,68,0.2)'
                                                    },
                                                    children: [
                                                        ap.wr,
                                                        "%"
                        ...
                        mapPerf.length > 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "mb-6",
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "flex items-center gap-2 mb-4",
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3",
                                    children: mapPerf.map((mp)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                            className: "rounded-xl border border-[#1e2a3a]/60 overflow-hidden transition hover:border-[#2d4a6f]/40 hover:shadow-lg hover:shadow-black/20",
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "relative h-20",
                                                    children: [
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("img", {
                                                            src: __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$game$2d$data$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["MAP_IMAGES"][mp.name],
                                                            alt: mp.name,
                                                        ...
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                            className: "absolute inset-0 bg-gradient-to-t from-[#0b1120] via-[#0b1120]/60 to-transparent"
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3753,
                                                            columnNumber: 23
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "p-3 text-center bg-gradient-to-b from-[#0a1628]/90 to-[#0d1117]/95",
                                                    children: [
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                            className: `text-xl font-extrabold tabular-nums ${mp.wr >= 50 ? "text-emerald-400" : "text-red-400"}`,
                                                            style: {
                                                                textShadow: mp.wr >= 50 ? '0 0 10px rgba(16,185,129,0.2)' : '0 0 10px rgba(239,68,68,0.2)'
                                                            },
                                                            children: [
                                                                mp.wr,
                                                                "%"
                        ...
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            ...
                            children: [
                                ...
                                historyLoading ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "rounded-xl border border-[#1e2a3a]/60 bg-[#0a1628]/80 p-8 flex justify-center",
                                    children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    className: "rounded-xl border border-[#1e2a3a]/60 bg-gradient-to-br from-[#0a1628]/80 to-[#0d1117]/90 p-10 text-center",
                                    children: [
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(AimloLogo, {
                                            size: 72,
                                    ...
                                    children: savedReports.slice(0, 5).map((entry)=>{
                                        ...
                                        return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                                            ...
                                            children: [
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    className: "relative h-14 w-14 shrink-0 overflow-hidden rounded-lg ring-1 ring-white/[0.06]",
                                                    children: [
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("img", {
                                                        ...
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                            className: `absolute inset-0 ${entry.won ? 'bg-emerald-500/10' : 'bg-red-500/10'}`
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3803,
                                                            columnNumber: 25
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    ...
                                                    children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("img", {
                                                        src: (0, __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$game$2d$data$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["agentImgUrl"])(entry.agent),
                                                        alt: "",
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    ...
                                                    children: [
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                            ...
                                                            children: [
                                                                ...
                                                                tag && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                                    className: "rounded bg-white/[0.04] px-1.5 py-0.5 text-[9px] font-semibold",
                                                                    style: {
                                                                        color: tag.color
                                                                    },
                                                                    children: tag.label
                                                        ...
                                                        miniInsight && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                            className: "mt-1 text-[10px] text-cyan-400/60",
                                                            children: miniInsight
                                                        }, void 0, false, {
                                                            fileName: "[project]/app/page.tsx",
                                                            lineNumber: 3816,
                                                            columnNumber: 41
                ...
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        ...
                        children: [
                            ...
                            filteredReports.length > 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    children: [
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("select", {
                                            ...
                                            className: __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$design$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ds"].selectBase + " !py-2 !text-xs",
                                            children: [
                                                ...
                                                uniqueMaps.map((m)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("option", {
                                                        value: m,
                                                        children: m
                                                    }, m, false, {
                                                        fileName: "[project]/app/page.tsx",
                                    ...
                                    children: [
                                        ...
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("select", {
                                            ...
                                            children: [
                                                ...
                                                uniqueAgents.map((a)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("option", {
                                                        value: a,
                                                        children: a
                                                    }, a, false, {
                                                        fileName: "[project]/app/page.tsx",
                    ...
                    filteredReports.length > 0 && (historyFilterMap || historyFilterAgent || historyFilterResult !== "all") && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                    ...
                    savedReports.length === 0 ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        ...
                        className: "space-y-2",
                        children: filteredReports.map((entry)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                                onClick: ()=>{
                                    setViewingReport(entry);
                                    setScreen("reportDetail");
                                },
                    ...
                    children: [
                        ...
                        vr.rounds.length > 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            ...
                            children: [
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: vr.rounds.map((r, i)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                                            onClick: ()=>{
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    ...
                                    children: [
                                        ...
                                        vr.rounds.map((r, i)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                id: `round-detail-${i}`,
                                                className: `${__TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$design$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ds"].card} p-4 border-l-2 ${r.result === "win" ? "border-l-emerald-500/40" : "border-l-red-500/40"}`,
                                                children: [
                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                        ...
                                                        children: [
                                                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                                ...
                                                                children: [
                                                                    ...
                                                                    " ",
                                                                    r.result === "win" ? l.wonLabel : l.lostLabel,
                                                                    r.survived ? ` ${__TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$game$2d$data$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["IC"].dot} ${l.survivedShort}` : ""
                                                                ]
                                                            }, void 0, true, {
                                                                fileName: "[project]/app/page.tsx",
                                                                lineNumber: 4047,
                                                            ...
                                                            !r.skipped && r.deathLocation && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                    ...
                                                    r.skipped ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                        ...
                                                        lineNumber: 4056,
                                                        columnNumber: 23
                                                    }, this) : r.feedback ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                        className: "space-y-2",
                                                        children: [
                                                            r.feedback.deathAnalysis && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                                children: [
                                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                            ...
                                                            r.feedback.enemyPatterns && r.feedback.enemyPatterns.length > 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                                children: [
                                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                            ...
                                                            r.feedback.nextRoundPlan && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                                children: [
                                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                        ...
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            ...
                            children: [
                                ...
                                vr.bestRound && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(ReportCard, {
                                ...
                                vr.decisionScore && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(ReportCard, {
                                    icon: __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$game$2d$data$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["IC"].diamond,
                ...
                children: [
                    ...
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        ...
                        children: [
                            ...
                            report.bestRound && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(ReportCard, {
                                icon: __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$game$2d$data$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["IC"].bolt,
                                color: "text-blue-400",
                                label: l.bestRound,
                            ...
                            report.decisionScore && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(ReportCard, {
                                icon: __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$game$2d$data$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["IC"].diamond,
                                color: "text-purple-400",
                                label: l.decisionScore,
...
_s2(Home, "Pn8/P7ygec/v5nJF0LHlrZ6yY1I=");
_c15 = Home;
...
//# sourceMappingURL=_01y.n0l._.js.map