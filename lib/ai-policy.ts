...
// OLGU DÜZELTMESİ — "ULT SAHİBİNİ KARIŞTIRMA" satırı (canlı-test #8, 2026-08-03,
// Icebox/Jett/Spike Rush): koç metni "Chamber seni blade ile öldürdü" dedi, silah
// aslında Judge'dı. NEDEN: buradaki eski kod-ad örneği modele birebir "Düşman ulti
// attıysa 'Raze ultisini/bıçak ultisini attı' de" diye ÖĞRETİYORDU — bu OLGUSAL
// OLARAK YANLIŞ; "Blade Storm" (bıçak ultisi) JETT'in ultisidir (kanıt:
// knowledge/agents/duelists/jett.md — "Bıçaklar tabanca hassasiyetinde silah"),
// Raze'ninki roketatar (knowledge/agents/duelists/raze.md — "Roketin izi görünür"),
// Chamber'ınki uzun-hat keskin nişancı (knowledge/agents/sentinels/chamber.md —
// "ultı bedava bir op"). Aynı maçta R5 DOĞRU çevrildi ("Raze'nin bıçak ultisiyle")
// çünkü katil raze'di ve model bu hazır köprüyü kullandı; R2'de katil chamber'dı,
// köprü yoktu ve ham "blade" token'ı kopyalandı → tutarsızlık. Örnek düzeltildi +
// çapraz-atıf açıkça yasaklandı. Ham token'ın prompt'a hiç girmemesi ise KÖK fix:
// bkz. lib/comp-weapon.ts normalizeKillerInfoForPrompt (defense-in-depth).
...
- RESMİ ULT KOD-ADI YASAK: "Blade Storm"/"Blade" YAZMA → "bıçak ultisi"; Showstopper/Run It Back/Empress vb. → "ult". Düşman ulti attıysa kod-ad verme, "<Ajan> ultisini attı" de.
- ULT SAHİBİNİ KARIŞTIRMA: "bıçak ultisi" (Blade Storm) YALNIZCA Jett'indir. Raze'ninki roketatar, Chamber'ınki uzun hat tutan keskin nişancı tüfeği. Bir ajanın ultisini BAŞKA ajana atfetmek YASAK. Katilin ultisinin ne olduğundan emin değilsen türünü hiç yazma — sadece "ultisini attı" de. "blade/knife/melee" gibi ham token'ları silah adı sanma, silah adı olarak YAZMA.
