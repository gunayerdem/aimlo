---
patch: "13.00"
verified: 2026-07-08
---

# AJAN: Sova

## 1. Rol Kimliği
Sova'nın silahı bilgi. Recon oku, shock okları ve drone — üçü de her tur tekrar tekrar elinde. Takım kör girdiği yere sen kör girme. Ne kadar çok nokta ezberlersen o kadar tehlikelisin. Nokta bilmeden sahaya çıkma. 13.00 ile recon okunun (imza yeteneğin) bekleme süresi 60'tan 50 saniyeye indi — tur içinde daha sık tararsın.

## 2. Temel Sorumluluklar
- Ezberlediğin recon noktalarından dövüş başlamadan düşmanın yerini öğren
- Girmeden önce drone'u sür — takım kör değil, gördüğü yere gitsin
- İki shock okunu (hasar oku) da spike kurulana kadar sakla — defuse'a oturan rakibi spike'ın üstünden vuran ok budur
- Ult'i sadece gördüğün ya da az önce ortaya çıkardığın hedefe at
- Her tur takıma somut bilgi ver: "B'de iki kişi, sol köşede" gibi

## 3. Sık Yapılan Hatalar
- Bilmediğin noktaya recon atmak — dart havada vurulur, tarama sıfır
- Drone'u fazla uzun sürüp gövdenin başında ölmek
- İki shock okunu (hasar oku) spike kurulmadan harcamak — defuse'a oturanı vuran ok bu, erken atarsan post-plant kozunu kendin kapatırsın (recon oku konumu açar, hasar vermez)
- Ult'i bilgisiz, körüne atmak
- Sadece saldırı noktası ezberleyip savunma noktası bilmemek
- Savunmada recon'u rakip daha açılmadan, çok erken atmak
- Taramanın sonucunu takıma söylememek — recon'u tek başına izleyip değerini öldürmek
- Recon'a o kadar güvenip silahla dövüşmeyi unutmak — Sova da bir tüfek taşır, körü körüne yetenek beklemekten ölme

## 4. Kalıp -> Anlam

**IF** recon'ların indiği an düzenli olarak vuruluyor
**MEANING** Herkesin bildiği yaygın noktalara atıyorsun — rakip ezbere kafadan vuruyor
**COUNTER** Tavan köşesine, kutunun arkasına, giriş yolundan bakışını çevirmeden vuramayacakları açıya at. Her site için en az iki ayrı yükseklikten nokta bil — biri kapanınca diğerine geç.
**WHY** Zor noktadaki dart rakibi ikilemde bırakır: ya dönüp dart'a bakar ve giriş açısını bırakır, ya da etiketlenmeyi kabul eder. Kolay noktada bu ikilem hiç doğmaz.

**IF** drone sürerken ya da hemen ardından ölüyorsun
**MEANING** Gövden açıkta duruyor ya da drone'u gereğinden uzun tutuyorsun
**COUNTER** Kapağın arkasına çekil, drone'u kısa sür, bir arkadaşın seni kollasın. Rotayı önceden nişanlanan açılardan geçirme — her tur aynı hattan sürme. Ölürsen aldığın bilgi çöpe gider.
**WHY** Kısa ve güvenli drone hem bilgiyi verir hem seni hayatta tutar. Açıkta sürdüğün uzun drone, rakibe bedava kafa hediye eder.

**IF** iki dart da spike kurulmadan bitti
**MEANING** Spike üstünde defuse'u engelleme şansın kalmadı
**COUNTER** Atakta alan açmak için zorunlu değilse dart'ları sakla. Spike kurulunca ezberlediğin noktadan at — kaybettiğin turları işte böyle çevirirsin.
**WHY** Spike üstündeki dart rakibi defuse ederken ya yerinden kalkmaya zorlar ya da hasar yedirir. Erken harcarsan bu kozu kaybedersin.

**IF** ult'i bilgisiz, clutch'ta körüne atıyorsun
**MEANING** Ult'i kör bir hasar makinesine çeviriyorsun
**COUNTER** Ult'i her zaman recon ya da drone bilgisiyle eşle. Tahmine değil, gördüğün kesin pozisyona at.
**WHY** Bilgiyle atılan ult neredeyse garanti hasar verir. Körüne atış, ult'i hiçbir pozisyon bilgisi olmadan harcar.

**IF** ana haritalarında site başına ikiden az recon noktası biliyorsun
**MEANING** Nokta dağarcığın zayıf — rakip seni okur, recon okunun değersizleşir
**COUNTER** Harita başına en az beş nokta bil: atak site başına iki, savunma site başına iki, bir de erken tur varsayılanı. Özel oyuna gir ve bu noktaları tek tek çalış.
**WHY** Geniş nokta dağarcığı her tur, her durumda güvenilir bilgi verir. Az nokta seni belirli sitelerde ya da savunmada işlevsiz bırakır.

**IF** recon attın, bilgi geldi ama sen yerinden kıpırdamıyorsun — takım da bilgiyi kullanmıyor
**MEANING** Bilgiyi alıp rafa kaldırıyorsun; recon, peşinden gelen bir eylem olmadan boşa gidiyor
**COUNTER** Dart ateşlenmeden önce planını kur: "ışınlandıysa B'ye basıyoruz", "boşsa A'ya giriyoruz". Bilgi gelir gelmez sesli söyle ve hemen yön ver. Recon bir başlangıç sinyali, bitiş değil.
**WHY** Recon'un tek değeri, ortaya çıkardığı pozisyona göre takımın hareket etmesi. Bilgi gelir ama kimse oynamazsa rakip yer değiştirir, tarama bayatlar ve dart bedavaya gitmiş olur.

**IF** hasar veren shock okunu köşe temizlemek ya da spike üstü yerine rastgele atıyorsun
**MEANING** Garantili hasar/temizleme aracını tahmine harcıyorsun
**COUNTER** Hasar okunu (shock) iki işe ayır: dar köşede saklanan rakibi açığa çıkar, ya da spike kurulduktan sonra defuse'a oturanı yerinden kaldır. Boş havaya değil, daralttığın açıya at.
**WHY** Bu dart küçük bir alana yoğun hasar basar. Bilinen bir köşeye ya da spike üstüne atılınca rakibi ya kımıldatır ya yaralar — ikisi de senin lehine. Rastgele atılınca sadece sesten ibaret kalır.

**IF** recon hep bir tur içinde aynı saniyede iniyor (özellikle savunmada erken)
**MEANING** Zamanlamanı okutuyorsun — rakip recon okunu duyunca takvimini biliyor
**COUNTER** Dart zamanlamasını turdan tura kaydır: bazı tur erken tara, bazı tur sakla ve push'u gördükten sonra reaktif at. Savunmada rakip henüz açılmadan atma — boş alan tararsın.
**WHY** Sabit zamanlama, rakibe dart'tan önce hazırlanma ve taramanın dışında durma fırsatı verir. Değişken zamanlama her seferinde gerçek bilgi yakalar.

**IF** ölümlerin round'un erken saniyelerinde, daha keşif okunu atarken geliyor
**MEANING** Dart'a nişan alırken silahın inik, gövden sabit ve açıktasın — atış noktan korumasız
**COUNTER** Dart'ı duvar ya da kapak arkasından, atış açını bir takım arkadaşın tutarken at; noktayı ezbere bil ki nişan süresi kısalsın
**WHY** Nişan alan Sova oyunun en savunmasız hedefi. Erken round'da açıkta duran keşifçi, takım daha bilgi alamadan düşmana ilk sayıyı hediye eder.

**IF** öldüğünde takımından trade gelmiyor
**MEANING** Takımın önünde ya da takımdan kopuk ölüyorsun — bilgi ajanı en önde ölmez
**COUNTER** Dart ve drone önde gitsin, gövden değil: duelist'in bir adım arkasında, trade mesafesinde ilerle
**WHY** Sen düşünce takımın gözü de düşer; recon'suz kalan takım kör dövüşür. Trade edilemeyen Sova ölümü çifte kayıptır — sayı da gider, bilgi de.

## 5. Saldırı Kullanımı
- Execute'tan önce ezberlediğin noktadan recon at, takım girmeden drone'u sür — takım gördüğü yere girsin.
- İki shock okunu spike kurulana kadar sakla; plant sonrası spike üstü lineup defuse'u keser.
- Ult'i recon ya da drone'un doğruladığı pozisyona at, tahmine değil.

## 6. Savunma Kullanımı
- Recon'u reaktif at: rakip açılmadan erken atarsan boş alan tararsın. Push emaresi gelince push yönüne at.
- Erken tarama hangi site'a yığıldıklarını gösterir — rotasyon çağrısını bilgiyle yap.
- Retake'te önce recon: kaç kişi nerede tutuyor gör; shock oklarını bilinen köşelere at, açıları daralt.

## 7. Harita Etkileşimleri
- **Ascent**: Açık gökyüzü recon'un sekerek A Main'den B Short'a ulaşmasına izin verir. A ve B site kapı önüne dart at — rakip kapıdan çıkarken hasar yer. Ult'i uzun koridorda bekleyen rakibe doğrudan geçir. Orta saha kontrolü kritik; oradan atılan bir recon iki siteyi birden okur, bunu takıma sesli ver.
- **Haven**: Üç site, üç kat nokta demek — A, B ve C için ayrı ayrı hazırlan. Uzun koridorlardan drone sür, rakibin yerini erkenden öğren. Üç bombsite'ın hangisinin boş olduğunu erken recon ile bul; rotasyon mesafeleri uzun, bilgiyi erken verirsen takım boş siteye basar.
- **Bind**: Dar tavanlar bazı recon açılarını kısar — duvardan sekme noktalarına yığıl. Teleporter çıkışına dart at, rakip geçerken hasar yer. Rotasyonlar teleporter'dan döner; recon teleport rotasyonunu açığa çıkarmada en güçlü silahın — "biri ışınlandı" çağrısını hemen yap.
- **Icebox**: A ve B site üst raflarına recon at — yukarıyı tutan savunmacı anında açığa çıkar. A site açık çatıdan gelen sekme noktaları aynı anda birden fazla rakibi tarar. Çok katmanlı, dikey bir harita; recon'u sadece zemini değil üst kademeleri de tarayacak açıdan at yoksa yarısı kör kalır.
- **Sunset**: Mid avlusu (courtyard) drone için ana hat — mid'i drone'la aç, tepede (mid top) tutanı recon'la etiketle. A main'den içeri sektirilen recon site'ı ve elbow'u birden tarar. B'de market açısını recon'la aç; shock oklarını spike sonrası market kapısından defuse'a oturana sakla.
- **Split**: Dar koridorlar recon'un kırılma riskini artırır — dart'ı site ortasına değil tavan köşelerine ve kapak arkalarına indir, kolay noktadaki dart anında vurulur. Mid her şeyin anahtarı: Mid Top'taki Op'u recon'la etiketle, drone'u Vent ya da Mail hattına sür — mid'i okuyan takım iki Tower'a da split açar. Tower (heaven) tutucusunu shock okuyla yerinden kaldır — retake'te yüksekliği söken en ucuz araç bu. Dar sitelerde spike üstü shock lineup'ı başka haritalardan da değerli: defuse'a oturanın kaçacak yeri yok.
- **Summit**: Haritaya dağılmış ventler recon okuna ekstra menzil ve sektirme verir — geniş haritanın uzak köşelerini bu hatlardan tara. Hasar okun bir duvar panelini anında kırar, recon oku ise paneli düşürmez; duvarı alanı aldıktan ya da plant'tan sonra indir, erken kırarsan kendi rotanı da kaparsın. Mid Fountain çapraz ateşine utility'siz peek atılmaz — önce recon at, Mid Bend ve B Link taraflarından kim tutuyor gör. B push'unda B Tower'ın derin açısını taramadan takımı sokma; plant'tan sonra post-plant'ı ult ile tara.
- **Corrode**: Uzun mid şeridi (Top Mid — Stairs — Bottom Mid) recon'un evi — hatta kimse çıkmadan önce tara, Op tutan varsa o hatta utility'siz çıkılmaz. Mid Window rotasyon kapısı: geçmeden önce recon ile açıyı doğrula — pencereyi kim tutuyorsa rotasyon hızını o kontrol eder. B execute'unda Tower'ı taramadan site zeminine yayılma; yukarıdan bakan savunucu recon'la açığa çıkmadan dikey açı kapanmaz. A tarafında Elbow ve Pocket köşelerini drone ile temizle — iki köşe temizlenmeden hiçbir A girişi güvenli değil.
- **Breeze**: Rework Mid'i koridorlara böldü ama recon değerin düşmedi: Mid-A bağlantısındaki küçük pencere recon utility'si için bilerek bırakıldı — A taramanı o hattan kur. A Main hâlâ uzun bir hat: takım çıkmadan önce recon at, Op'u açığa çıkar. Çift kapının (Doors) sesi bedava tetikleyici — ses gelince drone'u Mid koridorlarına sür, gerçek geçiş mi fake mi ayırt et. Eski açık-alan sekmeleri gitti; recon'u koridor ağızlarına ve site içine at, boş gökyüzüne değil.
- **Lotus**: Üç site, üç kat nokta demek — A, B ve C için ayrı recon noktası bil. A site'ta ağaç (tree) üstünden sekme recon site'ı tarar; dönen kapı sesi drone'un geliş sesini örter — kapı sesi duyulduğunda drone'u o hatta sür, geçeni işaretle. Shock oklarını spike sonrasına sakla: üç siteli haritada retake toplanması geç gelir, defuse penceresini kapatan ok turu bitirir.
- **Rotasyon dışı haritalar**: Pearl — B tarafı tavanı yüksek: B hall üstünden inen dart site'ı ve arka köşeleri birden tarar. Hangi harita olursa olsun mantık aynı: zor açıdaki dart ikilem yaratır, kolay açıdaki bayatlar.

## 8. Eşleşme Notları
- **Omen'e karşı**: Recon, Omen'in ışınlandığı yeri açığa çıkarır. Drone, smoke'un içinden takip eder ve yerini doğrular.
- **Cypher'a karşı**: Recon, teli tetiklemeden rakip pozisyonunu tarar — Cypher'ın bilgisiyle doğrudan yarışırsın.
- **Duelist ile**: Önce drone, sonra recon — duelist tam bilgiyle girer. Köşede bekleyen rakibe duelisti boş açıyla göndermezsin.
- **Viper'a karşı**: Recon görüş hattıyla çalışır — Viper'ın duvarının ve sisinin **içinden taramaz**, sis tarama hattını keser. Ama ult'in (üç enerji patlaması) duvar ve sis delip karşı tarafa hasar verir ve konum açar; shock okunu da sisin arkasındaki bilinen açıya yollayarak Viper'ı kurulumundan kaldırırsın. Viper'ı sisin dışına zorlamada güçlüsün — sisin içini değil, kenarını ve arkasını oku.

## 9. Oyuncuya Ne Söylenmeli

### İyi performans gösterirken
**Oyuncu Eylemi:** Recon'ları savunmacının, giriş yolundan bakışını çevirmeden vuramayacağı tavan ve duvar noktalarına indiriyorsun. İki shock okunu spike kurulana kadar saklıyorsun — defuse'a oturanı spike'ın üstünden vuran ok bu.
**Faydalanma Penceresi:** Savunmacı dart'a dönerse giriş açısından kopar — takımına giriş penceresi açılır; görmezden gelirse etiket pozisyonunu tüm takımına önceden verir. İki seçenek de senin lehine. Spike sonrası iki dart, ulaşamayacakları yerden defuse'u engeller.
**Adaptasyon okuma:** Recon'un indiği noktayı önceden nişanlamaya başlarlarsa — farklı noktaya inen alternatif dart kullan. Taramanın dışında durmaya başlarlarsa — onları tahmin edilebilir bir alana itiyorsun, bunu takıma söyle. Dart inmeden erken push atarlarsa — noktayı atla, drone'u reaktif sür ve pozisyonu takıma bildir.

### Zorlanırken
"Dart'ların düşük değerli yerlere iniyor. Özel oyuna gir, noktalarını temizle. Nokta olmadan Sova, boş el çantası taşımaktan farksız."

"Recon'u atıyorsun ama peşinden gelen bir plan yok. Dart'ı ateşlemeden önce kararını ver: bilgi gelince ne yapacağını söyle, sonra at. Tek başına izlenen recon boşa gider."

"Yeteneklere fazla yaslanıp dövüşmeyi bırakmışsın. Recon bittiğinde de tüfeğin elinde — açıyı tut, kafadan vurmaya hazır dur. Sova bilgi verir ama turu kapatan yine senin nişanın."

### Tahmin edilebilir olunca
"Rakip dart noktalarını ezberden vuruyor. Aynı site için alternatif nokta lazım — en az iki seçeneğin olsun ki otopilotta seni geçemesinler."

"Recon'u her tur aynı saniyede atıyorsun, rakip takvimini ezberledi. Bazı tur erken tara, bazı tur sakla ve push'u görünce reaktif at — zamanlamanı bozarsan taraman yine canlı bilgi yakalar."

## 10. Bu Ajana Karşı
- Keşif oku indiğinde durduğun bölgeyi işaretlenmiş say — dart sesini duyduğun an pozisyon değiştir; eski yerinde kalırsan taranmış açıya gelen utility ve hazır crosshair seni bulur. Dart'ı görüyorsan vur, göremiyorsan görüş hattından çık.
- Drone'a karşı bekleme disiplini: drone seni işaretlemeden vur ya da köşeden tamamen çık; işaretlendiysen o pozisyonda durma — drone'un hemen arkasından takım gelir. Boş açıdaysan drone'a sıkma, ateş sesi yerini ele verir.
- Ult dalgaları duvardan geçer — sesi duyduğun an bulunduğun hattan yana kay, dar siperde sabit durma.
- Spike kurulduysa Sova hasar oklarını defuse üstüne saklar — tek seferde tam defuse zorlama: yarım defuse ile okunu üstüne çektir, ok bitince temiz bitir.
